...
│   ├── email_router.py     # Email функциональность (старый стиль)
│   ├── email_router/       # Email функциональность (новый стиль, независимый модуль)
│   │   ├── __init__.py     # Экспортирует router
│   │   ├── router.py       # Логика роутера
│   │   ├── messages.py     # Сообщения только этого модуля
│   │   ├── config.py       # Локальный конфиг (если нужен)
│   │   ├── keyboards.py    # Клавиатуры только этого модуля
│   │   └── services/       # Бизнес-логика только этого модуля
